...
class MaskedContextLinear(nn.Linear):
    """A linear module with a masked weight matrix.
    Allows for context features to be passed in and cat'd with the input."""
...
class MaskedContextFeedforwardBlock(nn.Module):
    ...
    def __init__(
        ...
        # Masked linear.
        self.linear = MaskedContextLinear(
            in_degrees=in_degrees,
            out_features=features,
            autoregressive_features=autoregressive_features,
...
class ContextMADE(nn.Module):
    ...
    def __init__(
        ...
        # Initial layer.
        self.initial_layer = MaskedContextLinear(
